...
function groupByDay(photos: Photo[], episodes: Episode[]): GroupedPhotos[] {
  const groups: Record<string, Photo[]> = {}
  for (const photo of photos) {
    const key = (photo.taken_at || photo.uploaded_at).slice(0, 10)
    if (!groups[key]) groups[key] = []
    groups[key].push(photo)
  }
  return Object.entries(groups)
...
export default function PhotosPage() {
  ...
  function addFilesToQueue(files: FileList | File[]) {
    const newItems: QueuedPhoto[] = Array.from(files).map(f => ({
    ...
    return (
      <div>
        <style>{`
          ...
          .mobile-photo { width: 100%; border-radius: 4px; display: block; object-fit: cover; cursor: pointer; }
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
          <div>
            <h1 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 28, fontWeight: 400, color: '#1C1A17', margin: '0 0 4px' }}>Photos</h1>
        ...
        {/* Mobile upload panel — shown when files are queued */}
        {queue.length > 0 && (
          <div style={{ backgroundColor: '#fff', border: '1px solid #D8D2C8', borderRadius: 4, padding: 16, marginBottom: 20 }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              >
                {uploading ? `Uploading… (${mobileDoneCount}/${queue.length})` : `Upload ${mobilePendingCount} photo${mobilePendingCount !== 1 ? 's' : ''}`}
              </button>
            </div>
          </div>
        )}
        ...
        {loading ? (
          ...
          <p style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 18, color: '#B0A89E', textAlign: 'center', padding: '40px 0' }}>No photos yet.</p>
        ) : (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {group.photos.map(photo => (
                  <img
                    ...
                    onClick={() => openLightbox(photo, group.photos)}
                  />
                ))}
              </div>
            </div>
        ...
        <div style={{ marginBottom: 40 }}>
          <div className={`upload-zone ${dragOver ? 'drag-over' : ''}`}
            ...
            <input ref={fileInputRef} type="file" accept="image/*" multiple style={{ display: 'none' }}
              onChange={e => e.target.files && addFilesToQueue(e.target.files)} />
            <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 20, color: '#6B6359', marginBottom: 12 }}>Drop photos here or click to select</div>
            ...
            <div style={{ border: '1px solid #e8e0d5', borderRadius: 4, overflow: 'hidden', background: '#faf8f5' }}>
              ...
              <div style={{ padding: '12px 16px', borderTop: '1px solid #e8e0d5', display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
                ...
                <button onClick={handleUploadAll} disabled={uploading || pendingCount === 0}
                  ...
                  {uploading ? `Uploading… (${doneCount}/${queue.length})` : `Upload ${pendingCount} photo${pendingCount !== 1 ? 's' : ''}`}
                </button>
              </div>
            </div>
          )}
